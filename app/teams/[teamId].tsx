    ...
    <Card className="overflow-hidden p-0">
      ...
      >
        ...
        >
          {team.logo ? (
            ...
            <View className="flex-1 items-center justify-center">
              <Text style={{ color: headerSub, fontSize: 20, fontWeight: '600' }}>
        ...
        <View className="min-w-0 flex-1">
          <Text style={{ color: headerText, fontSize: 22, fontWeight: '600' }} numberOfLines={1}>
            {team.team_name}
          </Text>
        ...
        <View>
          <View className="flex-row border-t border-neutral-100">
            <View className="flex-1 border-r border-neutral-100 px-5 py-3.5">
              <Text className="text-xl font-medium text-neutral-900" style={NUM}>
                {stats.matches}
              </Text>
        ...
        <View className="border-t border-neutral-100 px-5 py-3.5">
          ...
          <View className="mt-2 flex-row flex-wrap" style={{ gap: 6 }}>
            {highlights.championships.seasons.map((s) => (
              ...
              >
                <Text className="text-[12px] font-medium text-amber-800">
                  🏆 {sanitizeLabel(s.season_name) || `시즌 ${s.year ?? ''}`}
                </Text>
              </View>
            ))}
          </View>
      ...
      {highlights && (highlights.top_appearances || highlights.top_scorer) && (
        <View className="flex-row border-t border-neutral-100">
          {highlights.top_appearances && (
            <Pressable
              className="flex-1 flex-row items-center border-r border-neutral-100 px-5 py-3.5 active:bg-neutral-50"
              style={{ gap: 8 }}
              onPress={() => router.push(`/players/${highlights.top_appearances!.player_id}`)}
            >
              {highlights.top_appearances.profile_image_url ? (
                <Image
                  source={{ uri: highlights.top_appearances.profile_image_url }}
          ...
          {highlights.top_scorer && (
            <Pressable
              className="flex-1 flex-row items-center px-5 py-3.5 active:bg-neutral-50"
              style={{ gap: 8 }}
              onPress={() => router.push(`/players/${highlights.top_scorer!.player_id}`)}
            >
              {highlights.top_scorer.profile_image_url ? (
                <Image
                  source={{ uri: highlights.top_scorer.profile_image_url }}
                  style={{ width: 32, height: 32, borderRadius: 16 }}
                  contentFit="cover"
                />
              ) : (
          ...
          <View className="px-5 pb-2 pt-4">
            ...
            {formation!.season_name && (
              <Text className="text-[12px] text-neutral-400">
                {sanitizeLabel(formation!.season_name)}
              </Text>
            )}
          </View>
                  ...
                  return (
                    <Pressable
                      ...
                      style={{
                        ...
                        left: `${leftPct}%`,
                        top: `${topPct}%`,
                        transform: [{ translateX: -BADGE_R }, { translateY: -BADGE_R }],
                        alignItems: 'center',
                        width: BADGE_R * 2,
                      }}
          ...
          <View className="mb-3 flex-row items-center justify-between px-2">
            ...
            <Text className="text-[12px] text-neutral-400">
              <Text className="font-medium text-emerald-600">
                {recentForm!.filter((m) => m.result === 'W').length}승
              </Text>
              <Text> · </Text>
              <Text className="font-medium text-red-600">
                {recentForm!.filter((m) => m.result === 'L').length}패
              </Text>
            </Text>
          </View>
              ...
              >
                ...
                <View className="flex-row items-center" style={{ gap: 6 }}>
                  ...
                  <View
                    ...
                    style={{ gap: 4 }}
                  >
                    <Text className="text-[11px] font-medium text-neutral-700" numberOfLines={1}>
                      {teamInfo.team_name}
                    </Text>
                    {teamInfo.logo ? (
                      <Image
                  ...
                  {/* 스코어 */}
                  <View
                    className="items-center justify-center rounded-md px-2.5 py-1"
                    style={{ backgroundColor: scoreBg, minWidth: 48 }}
                  >
                    <Text className="text-[12px] font-bold text-white" style={NUM}>
                      {myScore ?? 0} - {opScore ?? 0}
                    </Text>
                  </View>
                  {/* 상대팀 */}
...
function TopPlayersSection({
  ...
  if (
    !data ||
    (data.topScorers.length === 0 && data.topAssists.length === 0 && data.topRated.length === 0)
  )
    return null;

  const list =
    tab === 'scorers' ? data.topScorers : tab === 'assists' ? data.topAssists : data.topRated;
    ...
    <Card className="p-5">
      ...
      <View className="mb-3 flex-row rounded-lg bg-neutral-100 p-0.5">
        {(
          ...
          <Pressable
            key={key}
            className={`flex-1 items-center rounded-md py-1.5 ${tab === key ? 'bg-white' : ''}`}
            onPress={() => setTab(key)}
          >
            <Text
              className={`text-xs font-bold ${tab === key ? 'text-neutral-800' : 'text-neutral-400'}`}
            >
              {label}
            </Text>
          </Pressable>
      ...
      {list.slice(0, 5).map((p, i) => (
        <Pressable
          key={p.player_id}
          className={`flex-row items-center py-2.5 active:bg-neutral-50 ${i < Math.min(list.length, 5) - 1 ? 'border-b border-neutral-50' : ''}`}
          onPress={() => router.push(`/players/${p.player_id}`)}
        >
          <View
            className={`h-5 w-5 items-center justify-center rounded-full ${i === 0 ? 'bg-primary/10' : ''}`}
          >
            <Text
              className={`text-[10px] font-bold ${i === 0 ? 'text-primary' : 'text-neutral-400'}`}
            >
              {i + 1}
            </Text>
          </View>
          ...
          <Text className="ml-2 flex-1 text-sm font-medium text-neutral-700" numberOfLines={1}>
            {p.name}
          </Text>
          {i === 0 ? (
            <View className="rounded-full px-2 py-0.5" style={{ backgroundColor: teamColor }}>
              <Text className="text-xs font-bold" style={{ color: teamSecondaryColor, ...NUM }}>
                {tab === 'rated' ? p.value.toFixed(1) : p.value}
              </Text>
            </View>
          ) : (
            <Text className="text-sm font-bold text-neutral-700" style={NUM}>
              {tab === 'rated' ? p.value.toFixed(1) : p.value}
            </Text>
          )}
        </Pressable>
...
function CoachStatsSection({
  ...
  records: CoachSeasonRecord[] | undefined;
  teamColor: string;
}) {
  const VISIBLE = 3;
  const CHART_H = 140;
              ...
              return (
                ...
                >
                  <View style={{ position: 'absolute', top, alignItems: 'center' }}>
                    ...
                    >
                      <Text style={{ color: '#fff', fontSize: 13, fontWeight: 'bold', ...NUM }}>
                        {r.win_rate}%
                      </Text>
                    </View>
                    <View
                    ...
                    >
                      <Text style={{ color: '#6B7280', fontSize: 11, fontWeight: '500', ...NUM }}>
                        {r.ppg} 승점
                      </Text>
                    </View>
                  </View>
        ...
        <View className="flex-row py-3">
          {visible.map((r, i) => (
            <View key={`cp-${r.season_name}-${r.coach_id}-${i}`} className="flex-1 items-center">
              {r.profile_image_url ? (
                <Image
                  source={{ uri: r.profile_image_url }}
    ...
    <Card className="p-5">
      ...
      {displayed.map((p, i) => {
        ...
        return (
          <Pressable
            key={p.player_id}
            className={`flex-row items-center py-2.5 active:bg-neutral-50 ${i < displayed.length - 1 ? 'border-b border-neutral-50' : ''}`}
            onPress={() => router.push(`/players/${p.player_id}`)}
          >
            {p.profile_image_url ? (
              <Image
            ...
            <View className="ml-2 min-w-0 flex-1">
              ...
              {p.jersey_number != null && (
                <Text className="text-[10px] text-neutral-400">#{p.jersey_number}</Text>
              )}
            </View>
            <View className="flex-row" style={{ gap: 8 }}>
              {isTopApp ? (
                <View style={hlStyle}>
                  <Text style={{ color: '#fff', fontSize: 12, fontWeight: 'bold', ...NUM }}>
                    {p.totals.appearances}
                  </Text>
                </View>
              ) : (
                <Text
                  className="w-7 text-center text-[12px] font-bold text-neutral-700"
                  style={NUM}
                >
                  {p.totals.appearances}
                </Text>
              ...
              {isTopGoal ? (
                ...
                <Text className="w-7 text-center text-[12px] font-bold text-primary" style={NUM}>
                  {p.totals.goals}
                </Text>
              )}
              {isTopAssist ? (
      ...
      {players.length > 8 && (
        ...
        >
          <Text className="text-[13px] font-semibold text-neutral-500">
...
function getSeasonOutcome(
  ...
  if (league === 'super' && position === 6)
    return { label: '강등', emoji: '⬇️', bg: 'bg-red-100', text: 'text-red-800' };
  if (league === 'super' && position === 5)
    return { label: '승강PO', emoji: '↕️', bg: 'bg-blue-100', text: 'text-blue-800' };
  if (league === 'challenge' && position === 1)
    return { label: '승격', emoji: '⬆️', bg: 'bg-green-100', text: 'text-green-800' };
  if (league === 'challenge' && position === 2)
    return { label: '승강PO', emoji: '↕️', bg: 'bg-blue-100', text: 'text-blue-800' };
  if (league === 'cup' && position === 1)
    return {
      label: '우승',
      emoji: '🏆',
      bg: 'bg-white border-2 border-primary',
  ...
  if (league === 'playoff' && position === 1)
    return { label: '슈퍼행', emoji: '⬆️', bg: 'bg-violet-100', text: 'text-violet-800' };
  if (league === 'playoff' && position === 2)
    return { label: '챌린지행', emoji: '➡️', bg: 'bg-indigo-100', text: 'text-indigo-800' };
    ...
    <Card className="overflow-hidden p-0">
      ...
      <View className="px-4 pb-4">
        {rows.map((row, i) => {
          ...
          return (
            <View
              key={`${row.season_id}-${row.league}-${i}`}
              className={`flex-row items-center py-2.5 ${i < rows.length - 1 ? 'border-b border-neutral-50' : ''}`}
            >
              {/* 연도 */}
              <Text className="w-10 text-[12px] font-medium text-neutral-700" style={NUM}>
                {row.year}
              </Text>
              {/* 시즌 */}
              <Text className="flex-1 text-[11px] font-medium text-neutral-900" numberOfLines={1}>
                {sanitizeLabel(row.season_name)}
              </Text>
              {/* 리그 뱃지 */}
              ...
              <View className="w-12 items-center">
                {pb ? (
                  <View className={`rounded px-1.5 py-0.5 ${pb.bg}`}>
                    <Text className={`text-[10px] font-semibold ${pb.text}`}>
                      {pb.medal ? `${pb.medal} ${row.position}위` : `${row.position}위`}
                    </Text>
                  </View>
                ) : (
              ...
              {/* 승점 */}
              <Text
                className="w-8 text-center text-[12px] font-medium text-neutral-700"
                style={NUM}
              >
                {row.points}
              </Text>
              ...
              <View className="w-16 items-center">
                {outcome ? (
                  <View className={`rounded-full px-1.5 py-0.5 ${outcome.bg}`}>
                    <Text className={`text-[9px] font-semibold ${outcome.text}`}>
...
export default function TeamDetailScreen() {
  ...
  const {
    ...
    isError: teamError,
    refetch: refetchTeam,
  } = useQuery({ queryKey: ['teamById', id], queryFn: () => getTeamByIdPrisma(id) });
  ...
  const { data: formation } = useQuery({
    queryKey: ['teamFormation', id],
    queryFn: () => getTeamFormationPrisma(id),
  });
  const { data: recentForm } = useQuery({
    queryKey: ['teamRecentForm', id],
    queryFn: () => getTeamRecentFormPrisma(id, 10),
  });
  const { data: topPlayersData } = useQuery({
    queryKey: ['teamTopPlayers', id],
    queryFn: () => getTeamTopPlayersPrisma(id),
  });
  const { data: coachRecords } = useQuery({
    queryKey: ['teamCoachRecords', id],
    queryFn: () => getTeamCoachRecordsPrisma(id),
  });
      ...
      >
        <View className="px-4 pb-12 pt-4" style={{ gap: 16 }}>
          ...
          <FormationSection
            ...
            teamColor={teamColor}
            teamInfo={{ team_name: team.team_name, logo: team.logo }}
            router={router}
          />
          ...
          {(team.about || team.description) && (
            <Card className="p-5">
              ...
              <Text className="text-[13px] leading-5 text-neutral-600">
                {sanitizeLabel(team.about || team.description)}
              </Text>
            </Card>
          )}
        </View>
